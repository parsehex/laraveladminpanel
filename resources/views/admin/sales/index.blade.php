                        ...
                        <tr class="appliance-status-row {{ $statusClass }}">
                            ...
                            <td class="sticky-action px-4 py-3 text-right">
                                ...
                                @if($isSold)
                                    ...
                                    <button class="text-yellow-600 ml-2" data-edit-sale data-action="{{ route('admin.sales.sold-price.update', $item) }}" data-price="{{ $item->sold_price }}" data-sold-by="{{ $item->sold_by }}" data-sold-at="{{ $item->sold_at?->format('Y-m-d\TH:i') }}">Edit Price</button>
                                    @endcanAccess
                                @endif
                            </td>
                        </tr>
...
<div id="edit-sale-modal" class="hidden fixed inset-0 z-50 bg-black/50 items-center justify-center p-6">
    <div class="bg-white rounded-lg shadow max-w-md w-full p-6">
        ...
        <form method="POST" id="edit-sale-form" class="space-y-4">
            ...
            <div><label class="block text-sm font-medium text-gray-700 mb-1">Sold Date & Time</label><input type="datetime-local" name="sold_at" id="edit-sold-at" class="w-full px-3 py-2 border border-gray-300 rounded-md"></div>
...
<script>
    ...
    $('[data-edit-sale]').on('click', function () {
        ...
        $('#edit-sold-at').val($(this).data('sold-at') || '');
